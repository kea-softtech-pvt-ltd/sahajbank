...
export default function PastMedicationShow(props){
     const [MedicalDiseases, setMedicalDiseases] = useState([]);

    useEffect(() => {
        fetch("http://localhost:9000/fetchPastMedication",{
            method: 'GET',          
          })
            ...
            <ScrollView ontentContainerStyle={styles.contentContainer}> 
                {
                    MedicalDiseases.map((item,key)=>(
                        <View key={key}>                 
                            <Text style={styles.textColorContact}>{item.pastMedication}</Text>
                        </View>  
                    ))
                }  
            </ScrollView>
