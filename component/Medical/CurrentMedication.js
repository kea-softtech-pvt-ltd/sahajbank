...
export default function CurrentMedicationShow(props){
     const [MedicalDiseases, setMedicalDiseases] = useState([]);

    useEffect(() => {
        fetch("http://localhost:9000/fetchCurrentMedication",{
            method: 'GET',          
          })
            ...
            <ScrollView ontentContainerStyle={styles.contentContainer}> 
                {
                    MedicalDiseases.map((item,key)=>(
                        <View key={key}>                 
                            <Text style={styles.textColorContact}>{item.currentMedication}</Text>
                        </View>  
                    ))
                }  
            </ScrollView>
