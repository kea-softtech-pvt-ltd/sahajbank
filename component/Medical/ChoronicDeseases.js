...
export default function ChronicDiseasesShow(props){
     const [MedicalDiseases, setMedicalDiseases] = useState([]);

    useEffect(() => {
        fetch("http://localhost:9000/fetchChronicDiseases",{
            method: 'GET',          
          })
            ...
            <ScrollView ontentContainerStyle={styles.contentContainer}> 
                {
                    MedicalDiseases.map((item,key)=>(
                        <View key={key}>                 
                            <Text style={styles.textColorContact}>{item.chronicDiseases}</Text>
                        </View>  
                    ))
                }  
            </ScrollView>
